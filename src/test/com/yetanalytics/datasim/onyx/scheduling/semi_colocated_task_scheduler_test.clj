...
(defn one-group [replica] ; TODO: Unused
  (-> replica
      (assoc :groups [:g1])
...
(defn gen-replica
  ...
  (let [group-map (generate-group-and-peer-ids groups peers)]
    {:peers                (into [] (mapcat seq (vals group-map)))
     :peer-tags            (into {} (for [[_g ps] group-map
                                          p ps]
                                      [p []]))
     :groups-index         group-map
     :peer-sites           (into {} (for [[g ps] group-map
                                          p      ps]
                                      [p {:address (format "%s-addr" (name g))
                                          :port    40200}]))
     :groups               (into [] (keys group-map))
     :groups-reverse-index (into {} (for [[g ps] group-map
                                          p ps]
                                      [p g])),
     :pairs                (into {} (mapcat identity
                                            (for [[g0 _] group-map
                                                  [g1 _] group-map
                                                  :when  (not= g0 g1)]
                                              [[g0 g1]
